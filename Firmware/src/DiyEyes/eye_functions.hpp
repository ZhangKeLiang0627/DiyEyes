...
uint16_t oldIris = (IRIS_MIN + IRIS_MAX) / 2,
         newIris;
#endif
...
{ // Lower eyelid threshold value
    ...
    digitalWrite(eye[e].tft_cs, LOW);
    tft->startWrite();
    tft->setAddrWindow(eye[e].xposition, 0, 256, 256);
        ...
        for (screenX = 0; screenX < SCREEN_WIDTH; screenX++, scleraX++, irisX++, lidX += dlidX)
        {
            ...
            *(&pbuffer[dmaBuf][0] + pixels++) = p >> 8 | p << 8;
            ...
            if (pixels >= BUFFER_SIZE)
            {
                ...
                {
                    pbufferx2[i * 2] = pbuffer[0][i];
                    pbufferx2[(i * 2) + 1] = pbuffer[0][i];
                }
                tft->pushPixels(pbufferx2, 240);
                tft->pushPixels(pbufferx2, 240);
#endif
                // Serial.printf("%d\n", pixels);
                pixels = 0;
...
void frame(uint16_t iScale) // Iris scale (0-1023)
{
    ...
    drawEye(&tft, eyeIndex, iScale, eyeX, eyeY, n, lThreshold);
